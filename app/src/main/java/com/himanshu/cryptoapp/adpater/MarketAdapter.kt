...
class MarketAdapter(
    ...
    var list: List<CryptoCurrencyListItem>,
    ...
    override fun onBindViewHolder(holder: MarketViewHolder, position: Int) {
        ...
        holder.itemView.setOnClickListener {
            ...
            if (fragmentType == "Home") {
                ...
                findNavController(it).navigate(
                    MarketFragmentDirections.actionMarketFragmentToDetailsFragment(
                        item
                    )
                )
            } else if (fragmentType == "Watch") {
                findNavController(it).navigate(
                    WatchlistFragmentDirections.actionWatchlistFragmentToDetailsFragment2(
                        item
                    )
                )
            }
